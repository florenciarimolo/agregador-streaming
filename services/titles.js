...
import { hasUnexpectedCharacters, getPrimaryLanguageForRegion, } from '@/utils/language-detection';
import { LanguageIsoCode, DEFAULT_LANGUAGE, DEFAULT_LANGUAGE_ISO, LATIN_SCRIPT_LANGUAGE_ISO_CODES, } from '@/constants/languages';
/**
 ...
 */
function normalizeLanguageCode(code, region) {
    if (!code)
        return '';
 ...
 */
export function getTitleInLanguage(titleJsonb, language, userRegion, isImagePath = false) {
    ...
    const shouldCheckAlphabet = !isImagePath &&
        requestedLangCode !== primaryLangCode &&
        LATIN_SCRIPT_LANGUAGE_ISO_CODES.includes(requestedLangCode) &&
    ...
    if (titleText) {
        ...
        if (shouldCheckAlphabet) {
            const hasUnexpected = hasUnexpectedCharacters(titleText, requestedLangCode);
            if (hasUnexpected) {
                ...
                const primaryKey = Object.keys(titleJsonb).find((k) => k.startsWith(`${primaryLanguage}-`));
                if (primaryKey) {
    ...
    if (langCode && titleJsonb[langCode]) {
        ...
        if (import.meta.dev) {
            console.log(`[getTitleInLanguage] Using legacy format for language ${language}, found key: ${langCode}`);
        }
        return legacyText;
    }
    ...
    if (firstKey) {
        ...
        const isFallbackLatin = LATIN_SCRIPT_LANGUAGE_ISO_CODES.includes(fallbackLangCode);
        ...
        if (shouldCheckAlphabet && !isFallbackLatin) {
            const hasUnexpected = hasUnexpectedCharacters(fallbackText, requestedLangCode);
            if (hasUnexpected) {
 ...
 */
export async function insertTitle(data) {
 ...
 */
export async function getTitleByTmdbId(tmdbId, type) {
    const supabase = useSupabaseClient();
    return await supabase
        .from(TABLES.TITLES)
        .select('id')
 ...
 */
export async function getTitlesByTmdbIds(tmdbIds, language = DEFAULT_LANGUAGE, userRegion, titleTypes) {
    const supabase = useSupabaseClient();
    if (import.meta.dev) {
        console.log('[getTitlesByTmdbIds] Fetching titles for tmdbIds:', tmdbIds, 'with language:', language);
    }
    ...
    if (missingTmdbIds.length > 0 && titleTypes) {
        if (import.meta.dev) {
            console.log('[getTitlesByTmdbIds] Missing titles, fetching from TMDB:', missingTmdbIds);
        }
        ...
        const fetchPromises = missingTmdbIds.map(async (tmdbId) => {
            ...
            if (!type) {
                console.warn(`[getTitlesByTmdbIds] No type found for tmdb_id ${tmdbId}, skipping`);
                return null;
            }
            try {
                ...
                if (import.meta.dev) {
                    console.log(`[getTitlesByTmdbIds] Successfully fetched and inserted title ${tmdbId} from TMDB`);
                }
                return tmdbId;
            }
            catch (err) {
                console.error(`[getTitlesByTmdbIds] Error fetching title ${tmdbId} from TMDB:`, err);
                return null;
            }
        });
        ...
        const { data: reloadedData, error: reloadError } = await supabase
            ...
            .in(TITLES_COLUMNS.TMDB_ID, tmdbIds);
        if (reloadError) {
            console.error('[getTitlesByTmdbIds] Error reloading titles after TMDB fetch:', reloadError);
        }
        else if (reloadData) {
            // Use reloaded data which includes the newly fetched titles
            data = reloadedData;
            if (import.meta.dev) {
                console.log(`[getTitlesByTmdbIds] Reloaded ${data.length} titles from database after TMDB fetch`);
            }
        }
    }
    if (!data || data.length === 0) {
        if (import.meta.dev) {
            console.warn('[getTitlesByTmdbIds] No titles found in database for tmdbIds:', tmdbIds);
        }
        return { data: [], error: null };
    }
    if (import.meta.dev) {
        console.log('[getTitlesByTmdbIds] Found', data.length, 'titles in database');
        // Check what language keys exist in the first title
        if (data.length > 0) {
            const firstTitle = data[0];
            const titleJsonb = firstTitle.title;
            if (titleJsonb && typeof titleJsonb === 'object') {
                console.log('[getTitlesByTmdbIds] Sample title language keys:', Object.keys(titleJsonb));
                console.log('[getTitlesByTmdbIds] Looking for language:', language, 'exists:', !!titleJsonb[language]);
            }
        }
    }
    // Check which titles need language updates and fetch them in parallel
    const updatePromises = [];
    const titlesNeedingUpdate = [];
    data.forEach((title) => {
        const titleJsonb = title.title;
        const overviewJsonb = title.overview;
        const posterPathJsonb = title.poster_path;
        // Check if language is missing
        const hasTitleLanguage = Boolean(titleJsonb && typeof titleJsonb === 'object' && titleJsonb[language]);
        const hasOverviewLanguage = Boolean(overviewJsonb &&
            typeof overviewJsonb === 'object' &&
            overviewJsonb[language]);
        const hasPosterPathLanguage = Boolean(posterPathJsonb &&
        ...
        if (!hasTitleLanguage || !hasOverviewLanguage || !hasPosterPathLanguage) {
            ...
            const updatePromise = $fetch('/api/titles/update-language', {
                method: 'POST',
                body: {
                ...
                .catch((err) => {
                console.error(`Error updating language for title ${title.tmdb_id}:`, err);
    ...
    const titlesNeedingPrimaryLanguageFallback = [];
    const titlesWithLanguage = data.map((title) => {
        const titleJsonb = title.title;
        const overviewJsonb = title.overview;
        // Extract title first
        const extractedTitle = getTitleInLanguage(titleJsonb, language, userRegion);
        const extractedOverview = getTitleInLanguage(overviewJsonb, language, userRegion);
        ...
        if (extractedTitle === '' && requestedLangCode !== primaryLangCode) {
            ...
            const hasPrimaryLanguage = titleJsonb[primaryLanguageKey] ||
                titleJsonb[primaryLanguage] ||
                Object.keys(titleJsonb).some((k) => k.startsWith(`${primaryLanguage}-`));
        ...
        return {
            ...
            poster_path: getTitleInLanguage(title.poster_path, language, userRegion, true // isImagePath = true - don't check language for image paths
            ),
        };
    });
    ...
    if (titlesNeedingPrimaryLanguageFallback.length > 0) {
        if (import.meta.dev) {
            console.log(`[getTitlesByTmdbIds] Titles needing primary language (${primaryLanguageKey}) fallback (non-Latin detected but no primary language found):`, titlesNeedingPrimaryLanguageFallback.map((t) => ({
                tmdb_id: t.tmdb_id,
                needsOverview: t.needsOverview,
            })));
        }
        // Fetch primary language translations from TMDB in parallel
        const fetchPrimaryLanguagePromises = titlesNeedingPrimaryLanguageFallback.map(async (title) => {
            try {
                ...
                if (import.meta.dev) {
                    console.log(`[getTitlesByTmdbIds] Successfully fetched primary language (${primaryLanguageKey}) for title ${title.tmdb_id} from TMDB`);
                }
                return title.tmdb_id;
            }
            catch (err) {
                console.error(`[getTitlesByTmdbIds] Error fetching primary language (${primaryLanguageKey}) for title ${title.tmdb_id} from TMDB:`, err);
                return null;
            }
        });
        await Promise.all(fetchPrimaryLanguagePromises);
        // Reload titles from database after fetching primary language
        const primaryLanguageTmdbIds = titlesNeedingPrimaryLanguageFallback.map((t) => t.tmdb_id);
        const { data: reloadedData, error: reloadError } = await supabase
            .from(TABLES.TITLES)
            .select('id, title, type, poster_path, tmdb_id, overview, genres')
            ...
            titlesWithLanguage.forEach((title, index) => {
                ...
                if (reloaded) {
                    ...
                    titlesWithLanguage[index] = {
                        ...title,
                        title: getTitleInLanguage(reloaded.title, language, userRegion),
                        overview: getTitleInLanguage(reloaded.overview, language, userRegion),
                        poster_path: getTitleInLanguage(reloaded.poster_path, language, userRegion, true),
                    };
                }
            });
        }
 ...
 * @param language Language code in ISO/TMDB format (e.g., 'es-ES', 'ca-ES', 'eu-ES', 'gl-ES', 'en-US'). Defaults to 'es-ES'
 */
export async function getTitleByTmdbIdWithLanguage(tmdbId, type, language = DEFAULT_LANGUAGE, userRegion) {
    const supabase = useSupabaseClient();
    const result = await supabase
        .from(TABLES.TITLES)
        .select('*')
    ...
    let titleJsonb = data.title;
    let overviewJsonb = data.overview;
    let posterPathJsonb = data.poster_path;
    // Check if language is missing
    const hasTitleLanguage = titleJsonb && typeof titleJsonb === 'object' && titleJsonb[language];
    const hasOverviewLanguage = overviewJsonb &&
        typeof overviewJsonb === 'object' &&
        overviewJsonb[language];
    const hasPosterPathLanguage = posterPathJsonb &&
        typeof posterPathJsonb === 'object' &&
        posterPathJsonb[language];
    // If language is missing, fetch from TMDB and update
    if (!hasTitleLanguage || !hasOverviewLanguage || !hasPosterPathLanguage) {
        try {
            const response = await $fetch('/api/titles/update-language', {
                method: 'POST',
                body: {
                    tmdb_id: tmdbId,
                    type: type,
            ...
            if (response.success) {
                ...
                if (!reloadError && reloadedData) {
                    ...
                    const reloadedTitleJsonb = reloadedData.title;
                    const reloadedOverviewJsonb = reloadedData.overview;
                    const reloadedPosterPathJsonb = reloadedData.poster_path;
                    ...
                    if (response.poster_path && !hasPosterPathLanguage) {
                        if (!posterPathJsonb) {
                            ...
                            posterPathJsonb = data.poster_path;
                        }
                        else {
    ...
    const posterPathText = getTitleInLanguage(posterPathJsonb, language, userRegion, true // isImagePath = true - don't check language for image paths
    );
